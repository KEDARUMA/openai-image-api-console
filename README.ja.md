...
OpenAI Image API Console は、OpenAI の画像生成・画像編集 API をデスクトップから操作するための非公式ツールです。

Tauri + React + Vite で構成されており、macOS / Windows / Linux 向けのデスクトップアプリとしてビルドできます。
...
1. アプリを起動します。
2. 設定画面を開き、OpenAI API キーを入力します。
3. 必要に応じて API Keys / Billing ページを開き、API キーや課金状態を確認します。
4. モード、モデル、サイズ、品質、背景、出力形式などを選択します。
5. プロンプトや入力画像を指定して画像を生成します。
6. 生成された画像はローカルに保存され、履歴から再確認できます。
