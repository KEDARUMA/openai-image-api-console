...
アルファチャンネル付きの背景透過画像を出力する事ができます。さらに、GPT Image 2 を含む、現在公開されている 11 種類のモデルを選択して画像生成できます。テキストからの画像生成、入力画像を使った生成、マスク編集、出力設定、生成履歴も扱えます。Tauri + React + Vite で構成されており、macOS / Windows / Linux 向けに配布しています。
...
- アルファチャンネル付きの背景透過画像を出力
- GPT Image 2 を含む、現在公開されている 11 種類のモデルで画像生成
- テキストからの画像生成に対応
- 入力画像を使った画像生成に対応
- マスク画像を使った画像編集に対応
...
現在の Windows インストーラーはコード署名を行っていません。そのため、起動時に Microsoft Defender SmartScreen の警告が表示される場合があります。

<img src="docs/images/windows-smartscreen-warning.png" alt="Windows SmartScreen 警告" width="50%">

この警告が表示された場合は、`詳細情報` をクリックし、`実行` を選択してください。

## 警告が不安な場合は

インストーラーを使わず、このリポジトリを clone してローカルで起動できます。ダウンロードしたバイナリをインストールしなくても、同じアプリ機能を利用できます。下記の「開発」と「ビルド」セクションを参照してください。
